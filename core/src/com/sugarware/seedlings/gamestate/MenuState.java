...
extends GameState {
    ...
    public void draw() {
        ...
        this.g.begin();
        this.g.draw(this.bg, 0, 0, Gdx.graphics.getWidth(), (Gdx.graphics.getWidth() / (float)bg.getRegionWidth()) * Gdx.graphics.getHeight());
        this.font.draw(this.g, "Seedlings", 0.0f, (float)Gdx.graphics.getHeight() - 25.0f, 0, "Seedlings".length(), Gdx.graphics.getWidth(), 1, true, "...");
        int i = 0;
        while (i < this.options.length) {
            ...
            this.font2.draw(this.g, this.options[i], 0.0f, (float)Gdx.graphics.getHeight() - 40.0f * (float)(i + 4), 0, this.options[i].length(), Gdx.graphics.getWidth(), 1, true, "...");
            ++i;
        }
        this.g.end();
    }
    ...
    public void cursorMoved(Vector3 coords, int pointer) {
        int i = 0;
        while (i < this.options.length) {
            if (coords.y > (float)Gdx.graphics.getHeight() - 40.0f * (float)(i + 5) && coords.y < (float)Gdx.graphics.getHeight() - 40.0f * (float)(i + 5) + this.font2.getLineHeight()) {
                this.sel = i;
                break;
            }
            ++i;
