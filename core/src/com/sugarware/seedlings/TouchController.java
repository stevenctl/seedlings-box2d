...
implements InputProcessor {
    ...
    public boolean touchDown(int screenX, int screenY, int p, int button) {
        ...
        switch (this.scheme) {
            case 0: {
                ...
                if (x > Gdx.graphics.getWidth() - Gdx.graphics.getWidth() / 8 && x < Gdx.graphics.getWidth() && y > 0 && y < Gdx.graphics.getWidth() / 8) {
                    if (!(this.game.gsm.currentState instanceof PlayGameState)) break;
                ...
                if (x > Gdx.graphics.getWidth() - Gdx.graphics.getWidth() / 10 && x < Gdx.graphics.getWidth() && y > Gdx.graphics.getHeight() - Gdx.graphics.getWidth() / 10 && y < Gdx.graphics.getHeight()) {
                    this.handle(p, true, 37);
                    break;
                }
                if (x >= Gdx.graphics.getWidth() / 2 - 96 && x <= Gdx.graphics.getWidth() / 2 + 32 && y > Gdx.graphics.getHeight() - 80) {
                    this.handle(p, true, 48);
                    break;
                }
                if (x <= Gdx.graphics.getWidth() / 2) break;
                this.handle(p, true, 62);
                break;
            }
        }
    ...
    public boolean touchDragged(int x, int y, int p) {
        ...
        switch (this.scheme) {
            case 0: {
                if (x > 0 && x < Gdx.graphics.getWidth() / 10 && y > Gdx.graphics.getHeight() - Gdx.graphics.getWidth() / 10 && y < Gdx.graphics.getHeight()) break;
                ...
                if (x <= Gdx.graphics.getWidth() / 4 - Gdx.graphics.getWidth() / 8 || x >= Gdx.graphics.getWidth() || y <= 0 || y >= Gdx.graphics.getWidth() / 8 || this.pointer[p] != 62) break;
                this.touchUp(x, Gdx.graphics.getHeight() + y, p, -1);
                if (!(this.game.gsm.currentState instanceof PlayGameState)) break;
                PlayGameState st = (PlayGameState)this.game.gsm.currentState;
                if (st.p instanceof FireCharacter) {
    ...
    public void draw(SpriteBatch g) {
        ...
        if (this.game.gsm.currentState instanceof PlayGameState) {
            ...
            switch (this.scheme) {
                case 0: {
                    if (st.p instanceof FireCharacter) {
                        this.h.draw(this.btnAction, (float)(Gdx.graphics.getWidth() - Gdx.graphics.getWidth() / 8), 0.0f, (float)(Gdx.graphics.getWidth() / 8), (float)(Gdx.graphics.getWidth() / 8));
                    }
                    this.h.draw(this.btnPause, 0.0f, (float)(Gdx.graphics.getHeight() - Gdx.graphics.getWidth() / 10), (float)(Gdx.graphics.getWidth() / 10), (float)(Gdx.graphics.getWidth() / 10));
                    this.h.draw(this.btnLight, (float)(Gdx.graphics.getWidth() / 2), (float)(Gdx.graphics.getHeight() - 48), 48.0f, 48.0f);
                    break;
                }
            }
        }
