...
extends Entity {
    ...
    public void draw(SpriteBatch g) {
        if (this.shouldShow()) {
            ...
            g.begin();
            font.draw(g, this.hintText, 0.0f, (float)Gdx.graphics.getHeight() - font.getLineHeight() - 5.0f - font.getLineHeight() * (float)nhints, 0, this.hintText.length(), Gdx.graphics.getWidth(), 1, true, "...");
            g.end();
    ...
    private boolean shouldShow() {
        ...
        while (i < contactList.size) {
            ...
            if (contact.isTouching() && (contact.getFixtureA() == this.playerSensorFixture || contact.getFixtureB() == this.playerSensorFixture)) {
                ...
                ua = ((Object[])ua)[0];
                ub = ((Object[])ub)[0];
                if (!(contact.getFixtureA() == this.playerSensorFixture ? !(ub instanceof Player) : !(ua instanceof Player))) {
                    return true;
                }
            }
            ++i;
