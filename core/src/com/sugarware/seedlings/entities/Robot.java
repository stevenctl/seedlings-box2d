...
public class Robot extends Entity {
	...
	protected boolean isGrounded() {
		...
		while (i < contactList.size) {
			...
			if (contact.isTouching() && (contact.getFixtureA() == this.groundSensorFixture
					|| contact.getFixtureB() == this.groundSensorFixture)) {
				Object ua = contact.getFixtureA().getBody().getUserData();
				Object ub = contact.getFixtureB().getBody().getUserData();
				ua = ((Object[]) ua)[0];
				ub = ((Object[]) ub)[0];
				if (!(contact.getFixtureA() == this.groundSensorFixture ? !(ub + "").contains("world")
						: !(ua + "").contains("world"))) {
