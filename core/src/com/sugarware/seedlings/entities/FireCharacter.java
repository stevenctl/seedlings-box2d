...
public class FireCharacter extends Player implements Luminous {
		...
		if (dx == 0.0f && grounded && !this.jumping) {
			...
			if (this.shooting) {
				if (this.currentAction != 6) {
					boolean k = this.currentAction == 5 || this.currentAction == 7 || this.currentAction == 4
	...
	public void keyDown(int k) {
		...
		if (k == 38 && this.canshoot) {
			...
			this.gs.getEntities()
					.add(new Fireball(this.gs, this.body.getPosition().x, this.body.getPosition().y, this.facingRight));
	...
	public void updateLight() {
		if (this.light == null) {
			this.light = new PointLight(this.gs.rh, 500, lightCol, this.getWidth() * 4.0f, this.body.getPosition().x,
